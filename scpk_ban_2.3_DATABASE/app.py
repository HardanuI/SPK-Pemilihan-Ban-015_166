...
from db  import fetch_all, fetch_raw, insert_data, update_data, delete_data
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "📋 Dataset", "⚙️ Proses SAW", "🏆 Hasil Perangkingan",
    "📊 Visualisasi", "🗄️ Kelola Data", "👥 Profil Kelompok",
])
...
# ════════════════════════════════════════════════════════
# TAB 6 — KELOLA DATA (CRUD)
# ════════════════════════════════════════════════════════
with tab5:
    st.markdown('<h2 class="section-header">🗄️ Kelola Data Ban</h2>', unsafe_allow_html=True)
    st.markdown('<hr class="section-divider">', unsafe_allow_html=True)

    aksi = st.radio("Pilih Aksi", ["➕ Tambah Data", "✏️ Edit Data", "🗑️ Hapus Data"],
                    horizontal=True)

# ── TAMBAH ───────────────────────────────────────────
    if aksi == "➕ Tambah Data":
        st.markdown("#### Tambah Data Ban Baru")
        with st.form("form_tambah"):
            col1, col2 = st.columns(2)
            with col1:
                brand   = st.text_input("Merek Ban")
                model   = st.text_input("Model Ban")
                size    = st.text_input("Ukuran Ban (contoh: 185/65 R 15)")
                s_price = st.text_input("Harga Jual (Rp)")
            with col2:
                o_price = st.text_input("Harga Asli (Rp)")
                li      = st.text_input("Load Index")
                rating  = st.text_input("Rating (1.0 - 5.0)")
                velg    = st.text_input("Ukuran Velg (R)")
            submit = st.form_submit_button("💾 Simpan")

        if submit:
            # Validasi: semua field wajib diisi
            if not all([brand, model, size, s_price, o_price, li, rating, velg]):
                st.error("⚠️ Semua field wajib diisi.")
            else:
                try:
                    insert_data(
                        brand, model, size,
                        float(s_price), float(o_price),
                        int(li), float(rating), int(velg)
                    )
                    st.cache_data.clear()
                    st.success(f"✅ Data '{brand} {model} ({size})' berhasil ditambahkan!")
                    st.rerun()
                except ValueError:
                    st.error("⚠️ Harga, Load Index, Rating, dan Ukuran Velg harus berupa angka.")

    # ── EDIT ─────────────────────────────────────────────
    elif aksi == "✏️ Edit Data":
        st.markdown("#### Edit Data Ban")
        df_raw = fetch_raw()
        if df_raw.empty:
            st.info("Tidak ada data.")
        else:
            df_raw['label'] = (df_raw['tyre_brand'] + ' ' +
                               df_raw['model']      + ' (' +
                               df_raw['size']       + ')')
            pilihan = st.selectbox("Pilih data yang ingin diedit", df_raw['label'])
            row = df_raw[df_raw['label'] == pilihan].iloc[0]

            with st.form("form_edit"):
                col1, col2 = st.columns(2)
                with col1:
                    brand   = st.text_input("Merek Ban",  value=str(row['tyre_brand']))
                    model   = st.text_input("Model Ban",  value=str(row['model']))
                    size    = st.text_input("Ukuran Ban", value=str(row['size']))
                    s_price = st.text_input("Harga Jual (Rp)", value=str(row['selling_price']))
                with col2:
                    o_price = st.text_input("Harga Asli (Rp)",  value=str(row['original_price']))
                    li      = st.text_input("Load Index",        value=str(row['load_index']))
                    rating  = st.text_input("Rating (1.0 - 5.0)", value=str(row['rating']))
                    velg    = st.text_input("Ukuran Velg (R)",   value=str(row['velg_size']))
    ...
    # ── HAPUS ─────────────────────────────────────────────
        ...
        if df_raw.empty:
            ...
            df_raw['label'] = (df_raw['tyre_brand'] + ' ' +
                               df_raw['model']      + ' (' +
                               df_raw['size']       + ')')
            pilihan = st.selectbox("Pilih data yang ingin dihapus", df_raw['label'])
            row = df_raw[df_raw['label'] == pilihan].iloc[0]

            st.warning(f"⚠️ Kamu akan menghapus: **{pilihan}**")
            if st.button("🗑️ Ya, Hapus Data Ini", type="primary"):
